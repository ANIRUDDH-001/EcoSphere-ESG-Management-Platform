...
**DoD:** the 3-minute story runs flawlessly on prod (Vercel + Cloud Run + Supabase).
...
- **Vercel** ← `web/` (root dir `web`, `VITE_*` env). Auto-deploy on push to `main`.
- **Cloud Run** ← `api/` (Dockerfile, Cloud Build trigger on `main`). Secrets in Cloud Run env.
- **Supabase** — one project; migrations via `supabase db push`; `pg_cron` for overdue/reminders/snapshots.
...
prompts/
  ...
  baseline/            b00_01 … b00_14 + b00_E2E gate   ← LEAD executes first, then freezes/tags
  ...
  integration/         phase 99 sub-tasks + final submission gate
```
